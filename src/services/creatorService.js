...
const toCamelCase = (creator) => {
  ...
  return {
    ...
    regioni: creator.regioni || [],
    tier: creator.tier,
    cellulare: creator.cellulare,
    emails: creator.emails || (creator.email ? [creator.email] : []),
    residenza: creator.residenza,
    domicilioFiscale: creator.domicilio_fiscale,
    piva: creator.piva,
    codiceFiscale: creator.codice_fiscale,
    note: creator.note,
    noteLog: creator.note_log || [],
...
const toSnakeCase = (creator) => {
  return {
    nome: creator.nome,
    cognome: cleanValue(creator.cognome),
    ...
    fee: creator.fee ? parseFloat(creator.fee) : null,
    cluster: creator.cluster || [],
    regioni: creator.regioni || [],
    tier: cleanValue(creator.tier),
    cellulare: cleanValue(creator.cellulare),
    emails: creator.emails || [],
    residenza: cleanValue(creator.residenza),
    domicilio_fiscale: cleanValue(creator.domicilioFiscale),
    piva: cleanValue(creator.piva),
    codice_fiscale: cleanValue(creator.codiceFiscale),
    note: cleanValue(creator.note),
    note_log: creator.noteLog || [],
